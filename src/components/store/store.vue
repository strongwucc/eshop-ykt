    ...
    <div class="home-page" ref="homePage" :style="{visibility: !showLoading ? 'visible' : 'hidden'}">
      ...
      <div class="scroll-wrapper" ref="scrollWrapper">
        <div class="wrapper" ref="_wrapper">
          ...
          <p class="table-num">
            <span v-if="deskInfo && deskInfo.type_name && deskInfo.name">当前：{{deskInfo.type_name + ' ' + deskInfo.name}}</span>
          </p>
      ...
      <mt-popup
        ...
        <div class="goods-detail">
          <img class="goods-pic" :src="goodsPic"/>
        </div>
      </mt-popup>
...
<script>
  ...
  export default {
    ...
    components: {Goods, Floating, StoreInfo, GoodsSpecification},
    watch: {
      ...
      // mainScroll: function (val) {
      //   if (val) {
      //     this._mainScroll.enable()
      //     if (this.jsScroll) {
      //       this._mainScroll.scrollTo(0, -this.goodsListTop[this.catIndex], 400)
      //       this.jsScroll = false
      //     } else {
      //       if (!!this.touchStartX && this.touchStartX > this.elementsStyle.SIDE_WIDTH) {
      //         if (this.mainLengthDifference < 500 * dpr) {
      //           const scale = this.mainLengthDifference / (500 * dpr)
      //           this._mainScroll.scrollTo(0, -this.mainLengthDifference, 1400 * scale)
      //         } else {
      //           this._mainScroll.scrollTo(0, -500 * dpr, 1400)
      //         }
      //       }
      //     }
      //   } else {
      //     this._mainScroll.disable()
      //   }
      // },
    ...
    methods: {
      ...
      getStoreInfo () {
        this.$http.post(this.API.seller.store_info,{store_id:this.storeId,desk_id:this.deskId}).then(res => {
          if (res.return_code === '0000') {
            ...
            if (res.data.desk.status == '0') {
              this.$router.push('/people_num/'+this.storeId+'/'+this.deskId)
      ...
      //跳转至search页面
      goSearch () {
        this.$router.push({name: 'search', params: {storeId: this.storeId, deskId: this.deskId, peopleNum: this.peopleNum}})
      },
