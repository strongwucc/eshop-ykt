<template>
  <div class="dine-detail">
    <div class="head">
      ...
      <div class="order-status" v-else-if="order.status == 'active' && order.pay_status == '0'">
        <img src="../../../assets/img/order_detail/order_ic_zhifu@2x.png">
        <span>订单待付款</span>
      </div>
      <div class="order-status" v-else-if="order.status == 'active' && order.pay_status == '1'">
        <img src="../../../assets/img/order_detail/order_finish@2x.png">
        <span>订单已完成</span>
      </div>
      <div class="order-status" v-else-if="order.status == 'active' && order.pay_status >= 4">
        <img src="../../../assets/img/order_detail/order_ic_zhifu@2x.png">
        <span>退款成功</span>
      </div>
      <p class="refund-amount" v-if="order.status == 'active' && order.pay_status >= 4 && NP.minus(order.total_amount, order.payed) > 0">退款金额：￥{{NP.minus(order.total_amount, order.payed)}}</p>
    </div>
    <div class="main">
      <pending-payment :order-items="order.items" :desk-info="order.desk_info"></pending-payment>
      <div class="remark">
        <span>备注：</span>
        {{order.mark_text}}
      </div>
