<template>
  <div class="pay-page">
          ...
          <template v-if="payMethod.pay_type == '2'">
            <div v-for="card in cards" class="way member-card" @click.stop="selectCard(card)">
              <div class="l">
                ...
                <span class="txt">
                  <p>
                    <span v-if="card.member_card_type === '0'">会员卡</span>
                    <span v-if="card.member_card_type === '1'">实体卡</span>
                    <span>NO.{{card.card_no | cardNoFormat}}</span>
                  </p>
              ...
              <i
                class="selected"
                v-show="payType == 'card' && selectedCard.card_no == card.card_no"
              >
                <img src="../../assets/img/pay/option_select_yellow@2x.png" />
              </i>
              <i
    ...
    <mt-popup v-model="payViewVisible" popup-transition="popup-fade">
      <div class="pay-pwd-view">
        ...
        <i class="close" @click="payViewVisible = false">
          <img src="../../assets/img/common/pop_off_black@2x.png" />
        </i>
      </div>
    </mt-popup>
    <mt-popup v-model="cardPayViewVisible" popup-transition="popup-fade">
      <div class="card-pay-pwd-view">
        <div class="view-main">
          ...
          <div class="action-btn">
            <span class="cancel-btn" @click.stop="cardPayViewVisible = false">取消</span>
            <span class="pay-btn" @click.stop="cardPay">去支付</span>
          </div>
        </div>
      </div>
    </mt-popup>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      if (
        from.name === "quick_meal_confirm" ||
        from.name === "quick_meal_order" ||
        from.name === "quick_meal_detail"
      ) {
        vm.showNotice = true;
      }
    });
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      window.passGuard1 = new passGuard({
        mappurl:
          "http://ceshi4.sdykt.com.cn:1280/pos/card/getMapping/" +
          this.mappingId,
        maxLength: 8, // 最大输入长度
        ...
        rsaPublicKey:
          "" // rsa公钥
      });
  ...
  methods: {
    getPayMethod() {
      this.$http
        .post(this.API.order.pay_method, { order_id: this.orderId })
        .then(res => {
    ...
    goPay() {
      if (this.payType == "advance" && this.secret.length == 0) {
        ...
        this.$http
          .post(this.API.order.pay, {
            store_id: storeId,
            order_id: this.orderId,
            pay_type: this.payType,
            payment_code: Encrypt.encrypt(this.secret),
            h5_name: "ykt"
          })
          .then(res => {
            this.paying = false;
    ...
    deskStatus() {
      ...
      if (deskId) {
        this.$http
          .post(this.API.seller.desk_status, { desk_id: deskId })
          .then(res => {
            if (res.return_code === "0000") {
    ...
    cardPay() {
      ...
      $.ajax({
        url:
          "http://ceshi4.sdykt.com.cn:1280/pos/card/getRandkey?" +
          this.get_time(),
        type: "GET",
        async: false,
        success: function(ranKey) {
          ...
          _this.$myLoading.open({
            text: "加载中...",
            spinnerType: "fading-circle"
          });
          _this.loading = true;
          _this.$http
            .post(_this.API.order.card_pay, {
              order_id: _this.orderId,
              card_no: _this.selectedCard.card_no,
              password: password,
              key: ranKey,
              mappingId: _this.mappingId
            })
            .then(res => {
