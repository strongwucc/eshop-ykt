<template>
  <div class="card-recharge-page">
    <div class="member-NO">
      <div class="wrapper">
        <p class="txt">您充值的卡号</p>
        <p class="num">NO.{{cardNo}}</p>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      recharge () {
        ...
        this.$http.post(this.API.user.card_recharge, {store_id: storeId, no: this.cardNo, recharge_code: rechargeCode}).then(res => {
          if (res.return_code === '0000') {
